...
def doubly_stochastic(table, undirected=False, return_self_loops=False):
    ...
    table2 = table.copy()
    original_nodes = len(set(table["src"]) | set(table["trg"]))
    table = pd.pivot_table(table, values="nij", index="src",
                           columns="trg", aggfunc="sum", fill_value=0) + .001
...
def high_salience_skeleton(table, undirected=False, return_self_loops=False):
    ...
    G = nx.from_pandas_edgelist(
        table, source="src", target="trg", edge_attr="distance", create_using=nx.DiGraph())
...
def maximum_spanning_tree(table, undirected=False):
    ...
    table2.rename(columns={"source": "src",
                           "target": "trg", "nij": "score"}, inplace=True)
